...
.public_api_root <- specmill::script_root('check_public_api.R')
check_public_api <- function(root = .public_api_root, membership = dir.exists(file.path(root, 'dev'))) {
  selected <- if (membership) {
    ...
    vapply(specmill::inspect_client(root, callbacks = callbacks)$operations, `[[`, character(1), 'name')
  } else {
    NULL
  }
  specmill::check_public_boundary(root, file.path(.public_api_root, 'dev/specmill-public.yml'), selected)
}
if (sys.nframe() == 0L) {
