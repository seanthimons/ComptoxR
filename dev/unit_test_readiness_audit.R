...
audit_parse_namespace <- function(root = ".") {
  ...
  s3_methods <- sub("^\\s*S3method\\((.*)\\)\\s*$", "\\1", s3_lines, perl = TRUE)
...
audit_extract_cassette_references <- function(text) {
  matches <- gregexpr("use_cassette\\s*\\(\\s*['\"]([^'\"]+)['\"]", text, perl = TRUE)
...
audit_test_style <- function(root = ".", test_files = NULL) {
  test_files <- test_files %audit||% audit_list_files(root, "tests/testthat", "^test-.*\\.R$")
  ...
  for (i in seq_along(test_files)) {
    ...
    without_parameters[[i]] <- grepl("works without parameters", text, fixed = TRUE)
    using_vcr[[i]] <- grepl("\\buse_cassette\\s*\\(", text, perl = TRUE)
    using_mocked_bindings[[i]] <- grepl("\\b(with_mocked_bindings|local_mocked_bindings)\\s*\\(", text, perl = TRUE)
    using_skip_on_cran[[i]] <- grepl("\\bskip_on_cran\\s*\\(", text, perl = TRUE)
    using_skip_if_no_key[[i]] <- grepl("\\bskip_if_no_key\\s*\\(", text, perl = TRUE)
    using_skip_if_offline[[i]] <- grepl("\\bskip_if_offline\\s*\\(", text, perl = TRUE)
    mentioning_ctx_api_key[[i]] <- grepl("ctx_api_key", text, fixed = TRUE)
  ...
  reference_files <- lapply(unique_refs, function(ref) {
    sort(names(cassette_refs)[vapply(cassette_refs, function(x) ref %in% x, logical(1))])
  })
  ...
  list(
    ...
    generated_backticked_endpoint_call_files = sum(generated & backticked_endpoint),
    non_generated_backticked_endpoint_call_files = sum(!generated & backticked_endpoint),
    files_with_without_parameters_generated_case = sum(without_parameters),
    generated_files_with_without_parameters_case = sum(generated & without_parameters),
    non_generated_files_with_without_parameters_case = sum(!generated & without_parameters),
...
read_export_exclusions <- function(root = ".", path = audit_file_path(root, "dev/export_test_exclusions.json")) {
...
validate_export_exclusions <- function(exclusions, known_exports = NULL) {
  ...
  if (!identical(exclusions$artifact_schema, export_exclusion_schema)) {
    errors <- c(errors, sprintf("artifact_schema must be %s", export_exclusion_schema))
  }
  ...
  for (i in seq_along(entries)) {
    ...
    if (length(missing) > 0) {
      errors <- c(errors, sprintf("exclusions[%d] missing required field(s): %s", i, paste(missing, collapse = ", ")))
      next
    }
    ...
    if (length(empty) > 0) {
      errors <- c(errors, sprintf("exclusions[%d] has empty/non-scalar field(s): %s", i, paste(empty, collapse = ", ")))
      next
    }

    if (entry$export %in% seen) {
      errors <- c(errors, sprintf("duplicate exclusion for export: %s", entry$export))
      next
    }

    if (!is.null(known_exports) && !(entry$export %in% known_exports)) {
      errors <- c(errors, sprintf("exclusion references unknown function export: %s", entry$export))
    }
...
audit_test_contents <- function(root = ".", test_files) {
  contents <- lapply(test_files, function(file) audit_read_text(audit_file_path(root, file)))
  names(contents) <- test_files
  contents
}

classify_exports_against_tests <- function(function_exports, test_files, root = ".", exclusions = list()) {
  ...
  excluded_exports <- sort(vapply(exclusions, function(entry) entry$export, character(1)))
  ...
  records <- lapply(function_exports, function(export) {
    ...
    pattern <- paste0("(?<![A-Za-z0-9_.])", audit_regex_escape(export), "(?![A-Za-z0-9_.])")
    reference_files <- sort(names(contents)[vapply(contents, function(text) {
  ...
  without_named <- sort(function_exports[!vapply(records, `[[`, logical(1), "named_test_file")])
  without_reference <- sort(function_exports[!vapply(records, `[[`, logical(1), "literal_test_reference")])
  ...
  family_summary <- lapply(families, function(family) {
    family_records <- records[vapply(records, function(record) identical(record$family, family), logical(1))]
    list(
      exports = length(family_records),
      named_test_files = sum(vapply(family_records, `[[`, logical(1), "named_test_file")),
      test_references = sum(vapply(family_records, `[[`, logical(1), "literal_test_reference")),
      intentionally_excluded = sum(vapply(family_records, `[[`, logical(1), "intentionally_excluded")),
      without_test_reference = sum(!vapply(family_records, `[[`, logical(1), "literal_test_reference")),
      gaps_after_exclusions = sum(vapply(family_records, function(record) {
  ...
  list(
    ...
    exports_with_named_test_file = length(function_exports) - length(without_named),
    exports_without_named_test_file = length(without_named),
    exports_with_test_reference = length(function_exports) - length(without_reference),
...
audit_manifest_status <- function(root = ".") {
  ...
  retired <- isTRUE(manifest$retired) || identical(manifest$artifact_schema, "test_manifest_retired/v1")
  authoritative <- exists && !retired && !identical(manifest$authority, FALSE) && is.null(parse_error)
  legacy_files_total <- length(manifest$files %audit||% list())
  legacy_files_total <- manifest$legacy_manifest$files_total %audit||% legacy_files_total
  ...
  list(
    ...
    retired = retired,
    replacement = manifest$replacement %audit||% "dev/reports/unit_test_readiness_audit.json",
...
audit_scan_fixtures <- function(root = ".", fixture_files = NULL, cassette_references = character(0)) {
  fixture_files <- fixture_files %audit||% audit_list_files(root, "tests/testthat/fixtures", "\\.yml$", recursive = TRUE)
  ...
  refs_with_fixture <- cassette_references[paste0(cassette_references, ".yml") %in% fixture_basenames]
  refs_missing_fixture <- setdiff(cassette_references, refs_with_fixture)
  unreferenced_fixture_files <- sort(fixture_files[!(tools::file_path_sans_ext(fixture_basenames) %in% cassette_references)])
  ...
  for (file in fixture_files) {
    ...
    statuses <- suppressWarnings(as.integer(sub(".*status:\\s*([0-9]+).*", "\\1", status_lines, perl = TRUE)))
  ...
  list(
    ...
    fixture_files_not_referenced_by_literal_test = length(unreferenced_fixture_files),
...
audit_workflows <- function(root = ".", workflow_files = NULL) {
  workflow_files <- workflow_files %audit||% audit_list_files(root, ".github/workflows", "\\.ya?ml$")
  records <- lapply(sort(workflow_files), function(file) {
    ...
    list(
      ...
      runs_targeted_test_file = grepl("testthat::test_file\\s*\\(", text, perl = TRUE),
      runs_package_check = grepl("devtools::check\\s*\\(|R CMD check|check-r-package", text, perl = TRUE),
      uses_covr = grepl("covr::|package_coverage|codecov", text, perl = TRUE),
      uses_ctx_api_key_secret = grepl("ctx_api_key\\s*:.*secrets\\.|CTX_API_KEY|COMPTOX_API_KEY", text, perl = TRUE),
      has_workflow_dispatch = grepl("(^|\\n)\\s*workflow_dispatch\\s*:", text, perl = TRUE),
      has_pull_request = grepl("(^|\\n)\\s*pull_request\\s*:", text, perl = TRUE),
  ...
  list(
    ...
    workflows_running_devtools_test = sum(vapply(records, `[[`, logical(1), "runs_devtools_test")),
    workflows_running_targeted_test_file = sum(vapply(records, `[[`, logical(1), "runs_targeted_test_file")),
    workflows_running_package_check = sum(vapply(records, `[[`, logical(1), "runs_package_check")),
    workflows_using_covr = sum(vapply(records, `[[`, logical(1), "uses_covr")),
    workflows_using_ctx_api_key_secret = sum(vapply(records, `[[`, logical(1), "uses_ctx_api_key_secret"))
  )
}
...
audit_coverage_signals <- function(root = ".") {
  ...
  pipeline_text <- audit_read_text(audit_file_path(root, ".github/workflows/pipeline-tests.yml"))
  coverage_check_text <- audit_read_text(audit_file_path(root, ".github/workflows/coverage-check.yml"))
  ...
  list(
    ...
    codecov_project_target = if (grepl("target:\\s*auto", codecov_text)) "auto with 1% threshold" else "not detected",
    codecov_patch_target = if (grepl("target:\\s*80%", codecov_text)) "80% with 0% threshold" else "not detected",
    coverage_check_workflow = if (grepl("75", coverage_check_text)) "75% minimum, warn-only" else "not detected",
    pipeline_workflow_package_gate = if (grepl("75", pipeline_text)) "75% minimum, enforced" else "not detected",
    pipeline_workflow_dev_endpoint_eval_gate = if (grepl("80", pipeline_text)) "80% minimum, enforced" else "not detected",
    endpoint_coverage_baseline = audit_json_file(root, "schema/coverage_baseline.json"),
    badge_files = list(
      ccd = audit_json_file(root, ".github/badges/ccd-coverage.json"),
      chemi = audit_json_file(root, ".github/badges/chemi-coverage.json")
    )
  )
}

audit_testing_docs <- function(root = ".", current_test_files = NULL, generated_header_test_files = NULL) {
  docs <- c(".planning/codebase/TESTING.md", "dev/TESTING_GUIDE.md")
  records <- list()
  ...
  for (doc in docs) {
    ...
    if (grepl("CI has no API key", text, fixed = TRUE)) {
      issues <- c(issues, "Claims CI has no API key despite current secret-backed workflows.")
    }
    if (!grepl("unit_test_readiness_audit", text, fixed = TRUE)) {
      issues <- c(issues, "Does not point to dev/reports/unit_test_readiness_audit.json as the inventory source of truth.")
    }

    records[[doc]] <- list(
      file = doc,
      mentions_audit_artifact = grepl("dev/reports/unit_test_readiness_audit.json", text, fixed = TRUE),
      mentions_audit_command = grepl("dev/unit_test_readiness_audit.R", text, fixed = TRUE),
...
build_unit_test_readiness_audit <- function(root = ".") {
  ...
  fixture_files <- audit_list_files(root, "tests/testthat/fixtures", "\\.yml$", recursive = TRUE)
  workflow_files <- audit_list_files(root, ".github/workflows", "\\.ya?ml$")
  coverage_files <- sort(c(
    "codecov.yml",
    "schema/coverage_baseline.json",
    ".github/badges/ccd-coverage.json",
    ".github/badges/chemi-coverage.json"
  )[file.exists(audit_file_path(root, c(
    "codecov.yml",
    "schema/coverage_baseline.json",
    ".github/badges/ccd-coverage.json",
    ".github/badges/chemi-coverage.json"
  )))])
  docs <- sort(c(".planning/codebase/TESTING.md", "dev/TESTING_GUIDE.md")[
    file.exists(audit_file_path(root, c(".planning/codebase/TESTING.md", "dev/TESTING_GUIDE.md")))
  ])
  ...
  fixture_scan <- audit_scan_fixtures(root, fixture_files, style$cassette_references)
  ...
  list(
    ...
    inventory = list(
      ...
      test_style = style[setdiff(names(style), c("cassette_references", "cassette_reference_files"))],
      by_prefix = list(
...
write_unit_test_readiness_audit <- function(root = ".", output = audit_file_path(root, "dev/reports/unit_test_readiness_audit.json")) {
  report <- build_unit_test_readiness_audit(root)
  output_dir <- dirname(output)
  if (!dir.exists(output_dir)) {
    dir.create(output_dir, recursive = TRUE)
  }
  jsonlite::write_json(report, output, pretty = TRUE, auto_unbox = TRUE, null = "null", digits = NA)
  invisible(report)
}
...
unit_test_readiness_audit_main <- function(args = commandArgs(trailingOnly = TRUE), root = ".") {
  parsed <- audit_parse_args(args)
  output <- parsed$output %audit||% audit_file_path(root, "dev/reports/unit_test_readiness_audit.json")
  ...
  if (parsed$check_exports || parsed$fail_on_gaps) {
    if (length(export_gaps) > 0) {
      ...
      cat(paste0("  - ", head(export_gaps, 20), collapse = "\n"), "\n", sep = "")
    } else {
...
audit_is_script_entrypoint <- function() {
  file_args <- grep("^--file=", commandArgs(FALSE), value = TRUE)
  if (length(file_args) == 0) {
    return(FALSE)
  }

  script <- audit_norm_path(sub("^--file=", "", file_args[[length(file_args)]]))
  identical(basename(script), "unit_test_readiness_audit.R")
}

if (audit_is_script_entrypoint()) {
