...
#       foo <- file.info(list.files(
#         path = ".",
#         pattern = paste0("^", archive_string)
#       ))
#     } else {
...
#         speciesCommon == 'rat' |
#           speciesCommon == 'mouse' |
#           speciesCommon == 'rabbit' |
#           speciesCommon == 'guinea pig' |
#           speciesCommon == 'mouse, rat'
#       ) %>%
...
#       group_by(dtxsid) %>%
#       mutate(
#         Carcinogenicity = case_when(
#           Result = str_detect(
#             cancerCall,
#             'Known Human Carcinogen|potential occupational carcinogen|Likely Human Carcinogen|Likely to be Carcinogenic to Humans|Likely to be carcinogenic to humans|Carcinogenic to humans|Known|Group 1|Group 2A|Group A|Group B|B1|B2'
#           ) ~
#             'VH',
#           Result = str_detect(
#             cancerCall,
#             '(Possible human carcinogen)|2B|Group C|Reasonably Anticipated'
#           ) ~
#             'H',
#           Result = str_detect(
#             cancerCall,
...
#       arrange(
#         compound,
#         factor(Carcinogenicity, levels = c('VH', 'H', 'M', 'L'))
#       ) %>%
...
#           assayCategory == 'in vitro' |
#           assayCategory == 'in vivo'
#       ) %>%
#       group_by(dtxsid) %>%
...
#       pivot_wider(
#         id_cols = dtxsid,
#         names_from = assayCategory,
#         values_from = active,
#         values_fn = mean
#       ) %>%
#       mutate(
#         Mutagenicity = case_when(
...
#       arrange(
#         compound,
#         factor(Mutagenicity, levels = c('VH', 'H', 'M', 'L'))
#       ) %>%
...
#       group_by(dtxsid) %>%
#       transmute(
#         Endocrine_Disruption = case_when(
#           str_detect(reason, 'Endocrine disrupting properties') ~ 'H'
#         )
#       ) %>%
#       mutate(
#         endo_amount = case_when(
...
#           exposureRoute == 'dermal' & toxvalUnits == 'mg/kg-day' |
#           exposureRoute == 'inhalation' & toxvalUnits == 'mg/L' |
#           exposureRoute == 'inhalation' & toxvalUnits == 'mg/m3' |
#           exposureRoute == 'inhalation' & toxvalUnits == 'ppm'
#       ) %>%
#       dplyr::filter(
#         toxvalType == 'NOAEL' |
#           toxvalType == 'LOAEL' |
...
#           exposureRoute == 'oral' &
#             amount < 50 |
#             exposureRoute == 'dermal' & amount < 100 |
#             exposureRoute == 'inhalation' & amount < 1 ~
#             'H',
#           exposureRoute == 'oral' &
#             amount >= 50 &
#             amount <= 250 |
#             exposureRoute == 'oral' & amount >= 100 & amount <= 500 |
#             exposureRoute == 'inhalation' & amount >= 1 & amount <= 2.5 ~
#             'M',
#           exposureRoute == 'oral' &
#             amount > 250 |
#             exposureRoute == 'dermal' & amount > 500 |
#             exposureRoute == 'inhalation' & amount > 2.5 ~
#             'L',
#         )
#       ) %>%
#       as_tibble() %>%
...
#       arrange(
#         compound,
#         factor(Reproductive, levels = c('VH', 'H', 'M', 'L'))
#       ) %>%
#       distinct(compound, .keep_all = T) %>%
...
#       arrange(
#         compound,
#         factor(Developmental, levels = c('VH', 'H', 'M', 'L'))
#       ) %>%
#       distinct(compound, .keep_all = T) %>%
#       rename(developmental_amount = amount) %>%
#       mutate(
#         developmental_amount = case_when(
...
#       mutate(
#         Developmental = case_when(
#           Result = str_detect(Result, 'H360|H360Df|H360D|H360FD|H360Df|H362') ~
#             'H',
...
#       mutate(
#         'Acute_Aquatic_Toxicity' = case_when(
#           (amount < 1) ~ 'VH',
...
#       arrange(
#         compound,
#         factor(Acute_Aquatic_Toxicity, levels = c('VH', 'H', 'M', 'L'))
#       ) %>%
#       distinct(compound, .keep_all = T) %>%
...
#       summarize(compound = compound, amount = min(amount)) %>%
#       mutate(
#         'Chronic_Aquatic_Toxicity' = case_when(
#           (amount < 0.1) ~ 'VH',
...
#       arrange(
#         compound,
#         factor(Chronic_Aquatic_Toxicity, levels = c('VH', 'H', 'M', 'L'))
#       ) %>%
#       select(compound, Chronic_Aquatic_Toxicity) %>%
#       distinct(compound, .keep_all = T) %>%
#       mutate(
#         chronic_aq_amount = case_when(
...
#       group_by(compound) %>%
#       mutate(
#         Ignitability = case_when(
#           Result = str_detect(Result, 'H230|H231|H232|H250|H251|H252|H271') ~
#             'VH',
...
#       arrange(
#         compound,
#         factor(Ignitability, levels = c('VH', 'H', 'M', 'L'))
#       ) %>%
#       select(compound, Ignitability) %>%
#       distinct(compound, .keep_all = T) %>%
#       mutate(
#         ignite_amount = case_when(
...
#       arrange(
#         compound,
#         factor(propType, levels = c('experimental', 'predicted')),
#         factor(Ignitability, levels = c('VH', 'H', 'M', 'L'))
#       ) %>%
#       select(compound, Ignitability) %>%
#       distinct(compound, .keep_all = T) %>%
...
#       arrange(
#         dtxsid,
#         factor(valueType, levels = c('experimental', 'predicted'))
#       ) %>%
...
#         modelSource,
#         valueType
#       ) %>% #TODO Account for ECOTOX that has max/min values
#       arrange(
#         dtxsid,
#         desc(resultValue),
#         factor(endpointName, levels = c('experimental', 'predicted')),
#         factor(
#           endpointName,
#           levels = c('Bioaccumulation Factor', 'Bioconcentration Factor')
#         )
#       ) %>%
#       distinct(dtxsid, .keep_all = T) %>%
#       group_by(dtxsid) %>%
...
#       pivot_longer(
#         cols = predValMass:expValMass,
#         names_to = 'model',
#         values_to = 'bac_amount',
#         values_drop_na = TRUE
#       ) %>%
#       arrange(
#         factor(model, levels = c('expValMass', 'predValMass')),
...
#       summarise(
#         mobile_amount = mean(value),
#         .by = c(dtxsid, propertyId, propType)
#       ) %>%
#       arrange(
#         dtxsid,
#         propertyId,
...
#     cat(
#       '\nSearch data archived: ',
#       paste0(save_string, format(Sys.time(), "%Y-%m-%d_%H.%M.%S"), '.Rds'),
#       '\n'
#     )
