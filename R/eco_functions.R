...
#'   conversions applied. `organism_lifestage` contains the original ECOTOX
#'   code and `org_lifestage` contains its source description. No derived
#'   life-stage categories or reproductive flags are added.
#' @export
#' @family ecotox
#' @examples
#' \dontrun{
...
.eco_enrich_metadata <- function(query, con) {
  ...
  # Native ECOTOX lifestage code and description
  query <- query |>
    dplyr::left_join(
      dplyr::tbl(con, "lifestage_codes") |>
        dplyr::select("code", org_lifestage = "description"),
      by = dplyr::join_by("organism_lifestage" == "code")
    ) |>
    dplyr::relocate("org_lifestage", .after = "organism_lifestage")
