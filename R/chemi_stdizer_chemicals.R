...
#' }
chemi_stdizer_chemicals <- function(query, id_type = "AnyId", options = NULL, full = NULL) {
  ...
  req <- httr2::request(base_url) |>
    httr2::req_url_path_append("stdizer/chemicals") |>
  ...
  if (httr2::resp_status(resp) < 200 || httr2::resp_status(resp) >= 300) {
    cli::cli_abort("API request to {.val stdizer/chemicals} failed with status {httr2::resp_status(resp)}")
  }
