...
chemi_amos_mass_spectrum_similarity <- function(lower_mass_limit = NULL, methodology = NULL, spectrum = NULL, type = NULL, upper_mass_limit = NULL, window = NULL) {
  server <- "chemi_burl"
  req_data <- run_hook("chemi_amos_mass_spectrum_similarity", "pre_request", list(params = list(`lower_mass_limit` = lower_mass_limit, `methodology` = methodology, `spectrum` = spectrum, `type` = type, `upper_mass_limit` = upper_mass_limit, `window` = window, `server` = server)))
