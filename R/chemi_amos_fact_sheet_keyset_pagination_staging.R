...
#' @examples
#' \dontrun{
#' chemi_amos_fact_sheet_keyset_pagination_bulk_staging(limit = "DTXSID7020182")
#' }
chemi_amos_fact_sheet_keyset_pagination_bulk_staging <- function(limit, cursor = "", document_text = "", dtxsid = "", filterModel = structure(list(), names = character(0)), filters = structure(list(), names = character(0)), full_table = "", include_total = FALSE, quickFilter = "", sortModel = list(), all_pages = TRUE, max_pages = 100) {
  server <- "chemi_burl"
  req_data <- run_hook("chemi_amos_fact_sheet_keyset_pagination_bulk_staging", "pre_request", list(params = list(`limit` = limit, `cursor` = cursor, `document_text` = document_text, `dtxsid` = dtxsid, `filterModel` = filterModel, `filters` = filters, `full_table` = full_table, `include_total` = include_total, `quickFilter` = quickFilter, `sortModel` = sortModel, `all_pages` = all_pages, `max_pages` = max_pages, `server` = server)))
