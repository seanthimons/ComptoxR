		...
		imap(serv, function(idx, server){
			...
			# Sets the path
                ...
                if (ping_url(url_to_check)) {
									download.file(
										url = url_to_check, 
										destfile = here::here('schema', paste0("ctx_", endpoint,'_',server,'.json'))
									)
                }
	...
	map(endpoints, function(endpoint){
		...
		imap(serv, function(idx, server){
			...
			# Sets the path
                ...
                url_to_check <- paste0(Sys.getenv('chemi_burl'), "/", endpoint, '/api-docs')

                if (ping_url(url_to_check)) {
									download.file(
										url = url_to_check, 
										destfile = here::here('schema', paste0("chemi_",endpoint,'_', server,'.json'))
									)
                }
	...
	map(endpoints, function(endpoint){
		...
		imap(serv, function(idx, server){
			...
			# Sets the path
                ...
                url_to_check <- paste0(Sys.getenv('chemi_burl'), "/", endpoint, '/swagger.json')
