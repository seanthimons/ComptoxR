...
#' @examples
#' \dontrun{
#' chemi_amos_formula_staging(formula = "DTXSID7020182")
#' }
chemi_amos_formula_staging <- function(formula) {
  server <- "chemi_burl"
  req_data <- run_hook(
    "chemi_amos_formula_staging",
    "pre_request",
    list(params = list(`formula` = formula, `server` = server))
  )
