...
#' @examples
#' \dontrun{
#' chemi_amos_spectrum_count_for_methodology_staging(dtxsid = "DTXSID1024122")
#' }
chemi_amos_spectrum_count_for_methodology_staging <- function(dtxsid = NULL, spectrum_type = NULL) {
  server <- "chemi_burl"
  req_data <- run_hook("chemi_amos_spectrum_count_for_methodology_staging", "pre_request", list(params = list(`dtxsid` = dtxsid, `spectrum_type` = spectrum_type, `server` = server)))
