...
chemi_safety <- function(query) {
  ...
  data$nfpa <- df %>%
    ...
    mutate(
      ...
      special_bin = if_else(!str_detect(nfpa_special, "\\S"), NA_character_, nfpa_special)
    ) %>%
    select(
      -val,
      -starts_with("nfpa_")
    ) %>%
    pivot_longer(., cols = health_bin:special_bin, values_to = "bin", names_to = "hazard_class", values_drop_na = T) %>%
    mutate(
      val = case_when(
        bin == "OX" ~ "OX",
        bin == "W" ~ "W",
  ...
  df <- inner_join(data$ghs_codes, ghs_tbl, join_by("val" == "h_code")) %>%
    ...
    pivot_wider(., id_cols = dtxsid, names_from = class, values_from = bin, values_fill = NA) %>%
    rowwise() %>%
