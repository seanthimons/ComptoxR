hc_table2 <- function(query){
  ...
  data$final <- query %>%
    ...
    arrange(
      ...
      factor(finalScore.y, levels = c(
        ...
        'ND',
        NA)),
      factor(finalAuthority.y, levels = c(
        'Authoritative',
        'Screening',
        'QSAR Model',
        NA))
  ...
  data$records <- query %>%
    select(sid, hazardId.y,records_source, records_listType, records_score, records_valueMass, records_valueMassUnits, records_category, records_hazardCode) %>%
    arrange(
      factor(hazardId.y, levels = c(
        "acuteMammalianOral",
        "acuteMammalianDermal",
      ...
      factor(records_listType, levels = c(
        'Authoritative',
        'Screening',
        'QSAR Model',
        NA)),
      factor(records_score, levels = c(
        'VH',
        'H',
        'M',
        'L',
