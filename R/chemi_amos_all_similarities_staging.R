...
#' @examples
#' \dontrun{
#' chemi_amos_all_similarities_staging(da_window = "DTXSID1024122")
#' }
chemi_amos_all_similarities_staging <- function(da_window = NULL, dtxsids = NULL, min_intensity = NULL, ms_level = NULL, ppm_window = NULL, spectra = NULL) {
  server <- "chemi_burl"
  req_data <- run_hook("chemi_amos_all_similarities_staging", "pre_request", list(params = list(`da_window` = da_window, `dtxsids` = dtxsids, `min_intensity` = min_intensity, `ms_level` = ms_level, `ppm_window` = ppm_window, `spectra` = spectra, `server` = server)))
