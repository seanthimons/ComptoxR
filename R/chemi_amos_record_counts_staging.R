...
#' @examples
#' \dontrun{
#' chemi_amos_record_counts_staging(dtxsids = "DTXSID1024122")
#' }
chemi_amos_record_counts_staging <- function(dtxsids = NULL) {
  server <- "chemi_burl"
  req_data <- run_hook("chemi_amos_record_counts_staging", "pre_request", list(params = list(`dtxsids` = dtxsids, `server` = server)))
