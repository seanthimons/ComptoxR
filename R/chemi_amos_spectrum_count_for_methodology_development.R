...
#' @examples
#' \dontrun{
#' chemi_amos_spectrum_count_for_methodology_development(dtxsid = "DTXSID1024122")
#' }
chemi_amos_spectrum_count_for_methodology_development <- function(dtxsid = NULL, spectrum_type = NULL) {
  server <- "chemi_burl"
  req_data <- run_hook("chemi_amos_spectrum_count_for_methodology_development", "pre_request", list(params = list(`dtxsid` = dtxsid, `spectrum_type` = spectrum_type, `server` = server)))
