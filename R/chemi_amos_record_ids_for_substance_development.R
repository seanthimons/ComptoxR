...
#' @param record_type The record type of interest.. Type: string
...
#' @examples
#' \dontrun{
#' chemi_amos_record_ids_for_substance_development(dtxsid = "DTXSID7020182")
#' }
chemi_amos_record_ids_for_substance_development <- function(dtxsid, record_type = NULL) {
  server <- "chemi_burl"
  req_data <- run_hook("chemi_amos_record_ids_for_substance_development", "pre_request", list(params = list(`dtxsid` = dtxsid, `record_type` = record_type, `server` = server)))
