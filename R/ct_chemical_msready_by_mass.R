...
#' @examples
#' \dontrun{
#' ct_chemical_msready_by_mass_bulk(masses = "DTXSID7020182")
#' }
ct_chemical_msready_by_mass_bulk <- function(masses, error) {
  # Build request body
  body <- list()
  body$masses <- masses
  body$error <- error
...
#' @examples
#' \dontrun{
#' ct_chemical_msready_by_mass(start = "200.9")
#' }
ct_chemical_msready_by_mass <- function(start, end = NULL) {
  result <- generic_request(
    query = start,
    endpoint = "chemical/msready/search/by-mass/",
    method = "GET",
