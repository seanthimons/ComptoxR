...
#' @examples
#' \dontrun{
#' chemi_amos_for_document_ids_staging(record_type = "DTXSID7020182")
#' }
chemi_amos_for_document_ids_staging <- function(record_type, search_info, record_info_fields = NULL, table_fields = NULL) {
  server <- "chemi_burl"
  req_data <- run_hook("chemi_amos_for_document_ids_staging", "pre_request", list(params = list(`record_type` = record_type, `search_info` = search_info, `record_info_fields` = record_info_fields, `table_fields` = table_fields, `server` = server)))
