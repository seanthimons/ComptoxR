...
chemi_hazard <- function(
  ...
  if (missing(analogs)) {
    ...
    if (!missing(analogs) & length(analogs) > 1) {
      cli_abort('Only one analog method allowed')
    } else {
      if (!c(analogs %in% c('substructure', 'similar', 'toxprint'))) {
        cli_abort('Provided analog parameter not allowed!')
      } else {
  ...
  if (is.null(analogs)) {
    analogs_payload <- 'NULL'
  } else {
    analogs_payload <- analogs
  }
  if (is.null(min_sim)) {
    min_sim_payload <- 'NULL'
  } else {
    min_sim_payload <- min_sim
  }
  ...
  if (missing(coerce)) {
    ...
    coerce <- match.arg(coerce, choices = c('simple', 'bin', 'numerical'))
  }
  ...
  df <- data[2:3] %>%
    reduce(., left_join, by = common_cols) %>%
    select(-name) %>%
  ...
  data$data = df %>%
    select(data_id, dtxsid:hazardId, finalAuthority, finalScore, ) %>%
    arrange(
      factor(
        hazardId,
        levels = c(
    ...
    distinct(., dtxsid, hazardId, .keep_all = T) %>%
    mutate(
  ...
  switch(
    ...
    num = {
      ## Numerical----
      {
          ...
          mutate(
            ...
            valueMass = case_when(
              ...
              str_detect(source, 'T.E.S.T.') &
                str_detect(rationale, 'Positive for|Negative for') ~
                NA_integer_,
              #Cancer slope values
              str_detect(source, 'mid-Atlantic') &
                str_detect(rationale, 'SFO') ~
                NA_integer_,
              .default = valueMass
            )
          )
        ...
        ###check list----
        records_check <- left_join(temp_d, temp_r, by = 'data_id') %>%
          select(data_id)
        ...
        records$hcodes <- temp_df %>%
          ...
          mutate(
            amount = case_when(
              str_detect(endpoint, 'acuteMammalianOral') & finalScore == 'VH' ~
                50,
              str_detect(endpoint, 'acuteMammalianOral') & finalScore == 'H' ~
                175,
              str_detect(endpoint, 'acuteMammalianOral') & finalScore == 'M' ~
                1150,
              str_detect(endpoint, 'acuteMammalianOral') & finalScore == 'L' ~
                2000,

              str_detect(endpoint, 'acuteMammalianDermal') &
                finalScore == 'VH' ~
                200,
              str_detect(endpoint, 'acuteMammalianDermal') & finalScore == 'H' ~
                600,
              str_detect(endpoint, 'acuteMammalianDermal') & finalScore == 'M' ~
                1500,
              str_detect(endpoint, 'acuteMammalianDermal') & finalScore == 'L' ~
                2000,

              str_detect(endpoint, 'acuteMammalianInhalation') &
                finalScore == 'VH' ~
                2,
              str_detect(endpoint, 'acuteMammalianInhalation') &
                finalScore == 'H' ~
              ...
              str_detect(endpoint, 'carcinogenicity') & finalScore == 'VH' ~
                10000,
              str_detect(endpoint, 'carcinogenicity') & finalScore == 'H' ~
                1000,
              ...
              str_detect(endpoint, 'neurotoxicitySingle') & finalScore == 'H' ~
                500,
              str_detect(endpoint, 'neurotoxicitySingle') & finalScore == 'M' ~
                100,

              str_detect(endpoint, 'neurotoxicityRepeat') & finalScore == 'H' ~
                500,
              str_detect(endpoint, 'neurotoxicityRepeat') & finalScore == 'M' ~
                100,

              str_detect(endpoint, 'systemicToxicitySingle') &
                finalScore == 'H' ~
                500,
              str_detect(endpoint, 'systemicToxicitySingle') &
                finalScore == 'M' ~
              ...
              str_detect(endpoint, 'systemicToxicityRepeat') &
                finalScore == 'H' ~
                500,
              str_detect(endpoint, 'systemicToxicityRepeat') &
                finalScore == 'M' ~
              ...
              str_detect(endpoint, 'skinSensitization') & finalScore == 'H' ~
                100,

              str_detect(endpoint, 'skinIrritation') & finalScore == 'VH' ~
                1000,
            ...
            invert_flag = case_when(
              str_detect(
                endpoint,
                'acuteMammalianOral|acuteMammalianDermal|acuteMammalianInhalation|acuteAquatic|chronicAquatic'
              ) ~
                TRUE,
              .default = FALSE
            )
        ...
        records$num <- temp_df %>%
          ...
          mutate(
            invert_flag = case_when(
              str_detect(
                endpoint,
                'persistence|acuteMammalianOral|acuteMammalianDermal|acuteMammalianInhalation|acuteAquatic|chronicAquatic'
              ) ~
                TRUE,
              .default = NA
            )
        ...
        records$cat <- temp_df %>%
          ...
          mutate(
            amount = case_when(
              ...
              str_detect(endpoint, 'bioaccumulation') & finalScore == 'H' ~
                1000,
        ...
        records$nd <- temp_df %>%
          filter(finalScore == 'ND' | finalScore == 'I') %>%
          mutate(amount = NA_real_, invert_flag = FALSE) %>%
          select(data_id, endpoint, amount, invert_flag)
        ...
        if (nrow(records_temp) > 0)
          cli_abort('Filters failed!\nFile a bug report with reprex!')

        ##Records spreading----
        data$records <- left_join(
          select(data$data, data_id:name),
          records,
          by = 'data_id'
        ) %>%
          select(-data_id) %>%
