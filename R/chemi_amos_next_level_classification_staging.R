...
#' @examples
#' \dontrun{
#' chemi_amos_next_level_classification_staging(kingdom = "DTXSID1024122")
#' }
chemi_amos_next_level_classification_staging <- function(kingdom = NULL, klass = NULL, superklass = NULL) {
  server <- "chemi_burl"
  req_data <- run_hook("chemi_amos_next_level_classification_staging", "pre_request", list(params = list(`kingdom` = kingdom, `klass` = klass, `superklass` = superklass, `server` = server)))
