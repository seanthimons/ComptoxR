...
#' @param request.options.alertsInfo Optional parameter
...
chemi_alerts_staging <- function(query, idType = "AnyId", options = NULL, request.filesInfo = NULL, request.options.alerts = NULL, request.options.alertsInfo = NULL, request.options.resolve = NULL) {
  ...
  if ("request.options.alertsInfo" %in% names(req_data$params)) {
    request.options.alertsInfo <- req_data$params[["request.options.alertsInfo"]]
  }
