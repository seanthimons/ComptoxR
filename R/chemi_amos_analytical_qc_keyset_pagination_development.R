...
#' @examples
#' \dontrun{
#' chemi_amos_analytical_qc_keyset_pagination_development(limit = "DTXSID7020182")
#' }
chemi_amos_analytical_qc_keyset_pagination_development <- function(limit, cursor = NULL, all_pages = TRUE, max_pages = 100) {
  server <- "chemi_burl"
  req_data <- run_hook("chemi_amos_analytical_qc_keyset_pagination_development", "pre_request", list(params = list(`limit` = limit, `cursor` = cursor, `all_pages` = all_pages, `max_pages` = max_pages, `server` = server)))
...
#' @examples
#' \dontrun{
#' chemi_amos_analytical_qc_keyset_pagination_bulk_development(limit = "DTXSID7020182")
#' }
chemi_amos_analytical_qc_keyset_pagination_bulk_development <- function(limit, cursor = "", dtxsid = "", filterModel = structure(list(), names = character(0)), filters = structure(list(), names = character(0)), full_table = "", include_total = FALSE, quickFilter = "", sortModel = list(), all_pages = TRUE, max_pages = 100) {
  server <- "chemi_burl"
  req_data <- run_hook("chemi_amos_analytical_qc_keyset_pagination_bulk_development", "pre_request", list(params = list(`limit` = limit, `cursor` = cursor, `dtxsid` = dtxsid, `filterModel` = filterModel, `filters` = filters, `full_table` = full_table, `include_total` = include_total, `quickFilter` = quickFilter, `sortModel` = sortModel, `all_pages` = all_pages, `max_pages` = max_pages, `server` = server)))
  if (isTRUE(req_data$skip_request)) {
    return(req_data$result)
  }
  if ("limit" %in% names(req_data$params)) {
