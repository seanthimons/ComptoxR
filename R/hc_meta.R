#' Per compound data availability scoring
#'
#' Will return in the console the endpoint coverage before weighing.
#'
#' @param hc_table Takes a the result of a `hc_table()` function and returns a table of scores (0-1) on the amount of columns by percent each compound has.
#'
#' @return A tibble of results
#' @export

hc_compound_coverage <- function(hc_table){
  coverage_score <- hc_table %>%
    select(c(!contains('_amount'))) %>%
    mutate(data_coverage = (rowSums(is.na(.))))

  j = length(coverage_score)-1

  coverage_score <- coverage_score %>%
    mutate(data_coverage = 1-data_coverage/j) %>%
    select(compound, data_coverage)

  return(coverage_score)
}

#' Per endpoint - compound availability scoring
#'
#' @param hc_table Takes a the result of a `hc_table()` function and returns a table of scores (0-1) on the amount of compounds by percent each endpoint has.
#' @param filter A number(0-1) to cut off endpoint by for weighing. By default, sets to 0.5.
#'
#' @return A tibble of results
#' @export

hc_endpoint_coverage <- function(hc_table, filter = NA){
  j = nrow(hc_table)

  if(is.na(filter) == T){filt_score = 0.5}else(filt_score <- filter)

  endpoint_score <- hc_table %>%
    select(!c(compound)) %>%
    select(c(contains('_amount'))) %>%
    map( ~sum(is.na(.))) %>%
    as.data.frame() %>%
    mutate(across(everything(), ~ 1-(.x/j))) %>%
    pivot_longer(everything(), names_to = 'endpoint', values_to = 'score')
    print(endpoint_score)

    endpoint_filt_weight <- endpoint_score %>%
    filter(score >= filt_score) %>%
    mutate(weight = 1)

  return(endpoint_filt_weight)
}
