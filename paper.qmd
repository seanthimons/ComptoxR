...
#String search
ct_name('atraz', param = 'start-with') %>% select(searchName, rank, preferredName)

#InChlKey search
ct_name('MXWJVTOOROXGIU', param = 'start-with') %>% select(searchName, rank, preferredName)

#CAS_RN search
ct_name('1912-24', param = 'start-with') %>% select(searchName, rank, preferredName)
...
ct_prop(query = dtx_list[1], debug = FALSE) %>%
  ...
  summarize(value = round(mean(value), 3), unit) %>%
...
The function `hc_table()` function has a number of useful arguments that may be of interest. While the `query` argument is required, the `archive` or `save` arguments are not.
...
#| include: false
#| echo: false
df <- hc_table(query = dtx_list[c(1,7:10,29)], archive = T, save = F)
...
df_tbl %>% flextable() %>% 
  ...
  rotate(., j = 2:16, align = 'bottom', rotation = 'btlr', part = 'header') %>% 
  add_header_row(., values = c(' ','Human Health Effects','Ecotoxicity','Safety','Env. Fate'), colwidths = c(1,8, 2, 3, 2)) %>% 
  rotate(., i = 1,rotation = 'lrtb', align = 'bottom', part = 'header') 
```

Data abundance **possible change this** functions are also able to be ran to better assess the relative quality of both the compounds and endpoints being assessed.

The 'compound coverage' score is an assessment of how many endpoints for each compound were able to be queried for, displayed as a fraction of 1.0 (analogus to 100%). Here, all compounds that were included in the initial query has at-least 50% of the endpoints represented (n = 11)
...
For each endpoint from the hazard comparison table, the responses are scaled through a min-max normalization \[0-1\] (Eq 1). The score for each endpoint is then summed per compound, yielding a unit-less score, where the higher score indicates higher, relative hazard and risk (Eq 2).
...
As an example, a subset from an included dataset from the package (`dtx_list` ) An analysis of this suggests that along the endpoints we have selected for (n = 11), benzene shows the highest relative risk, with (Z)-1,2-Dichloroethylene showing the lowest relative risk, when comparing along similar exposure amounts. This scoring is partially due to the amount of data being returned by each compound, with benzene having \~81% of the endpoints documented, and (Z)-1,2-Dichloroethylene having \~56% (see @tbl-hc-compound-coverage)
...
If we were to re-run the analysis to *only* examine endpoints that all shared inclusive coverage of all of the compounds, the results are detailed below. Interestingly, Tamoxifen and Dazomet trade places within the list, indicating some overlapping parameter that may be of interest in the right context. While we recognize that this subset of compounds is not a realistic comparison, it still outlines the useful and subjectiveness of using the ToxPi approach for prioritization.
...
ra %>% select(name, effect_desc, observation, `pred resp`) %>% flextable()
```

Should a user desire it, a generic read-across function `read_across` is provided to generate a prediction based on other data (e.g.: LD50) leveraging the GenRA's ability to fingerprint and find similar compounds.
...
#tp <- left_join(tp, df_names, by = c('compound' = "preferredName")) %>% select(-compound)

max_num <- max(tp[,2:19], na.rm = T) 

tp %>% flextable() %>% 
  align(align = 'center', part = 'all') %>% 
  valign(j = 1, valign = 'bottom', part = 'header') %>%
  bg(j = 2:ncol(tp), bg = scales::col_numeric(palette = 'YlOrRd', domain = c(0,max_num)), part = 'body') %>% 
  bg(j = 'compound', bg = 'white')
```
