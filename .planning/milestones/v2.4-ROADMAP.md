...
**Phases:** 34-39, including inserted phases 36.1 and 36.2
**Plans:** 12/12 complete
**Archive created:** 2026-05-05

## Completion Summary

v2.4 replaced the v2.3 regex-first ECOTOX lifestage harmonization with a source-backed patch seed workflow and finalized runtime output from `eco_results()`. The milestone removed the old `ontology_id` surface, added deterministic in-place lifestage patching, shipped maintainer curation/provenance artifacts, and added mocked provider quality gates for OLS4, NVS, and BioPortal.

## Source Data Note

Rows with blank ECOTOX lifestage after runtime enrichment are source-data gaps where `tests.organism_lifestage` is empty in ECOTOX. The runtime join path is correct: `tests.organism_lifestage` -> `lifestage_codes.code` -> `lifestage_dictionary.org_lifestage`.
