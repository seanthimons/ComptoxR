...
if (!rebuild_is_needed) return(invisible(output_path))
...
release_files <- list.files(eco_files_dir, pattern = "release|ASCII|Ascii",
                            full.names = TRUE)
if (length(release_files) > 0) file.remove(release_files)
...
purrr::walk(main_txts, function(txt_path) {
  ...
  DBI::dbWriteTable(eco_con, tbl_name, arrow::read_parquet(pq_path),
                    overwrite = TRUE)
}, .progress = TRUE)
...
DBI::dbWriteTable(eco_con, "duration_conversion", duration_conversion,
                  overwrite = TRUE)
...
DBI::dbWriteTable(eco_con, "dict_test_result_duration",
                  test_result_duration_dictionary, overwrite = TRUE)
...
DBI::dbWriteTable(eco_con, "dict_risk_binning", risk_binning_rules,
                  overwrite = TRUE)
...
units_intermediate <- units_intermediate |>
  ...
  dplyr::mutate(
    ...
    part_counts = rowSums(!is.na(dplyr::select(
  ...
  tidyr::unite("cur_unit", dplyr::starts_with("unit_conv_u_"),
               sep = "/", na.rm = TRUE) |>
  tidyr::unite("cur_unit_type", dplyr::starts_with("type_u_"),
               sep = "/", na.rm = TRUE) |>
  dplyr::mutate(
...
DBI::dbWriteTable(eco_con, "z_unit_intermediate", units_intermediate,
                  overwrite = TRUE)
DBI::dbWriteTable(eco_con, "unit_conversion", unit_conversion,
                  overwrite = TRUE)
...
DBI::dbWriteTable(eco_con, "lifestage_dictionary", life_stage)
...
DBI::dbWriteTable(eco_con, "effect_groups_dictionary", effect_conversion,
                  overwrite = TRUE)
...
persist_con <- DBI::dbConnect(duckdb::duckdb(), dbdir = output_path,
                              read_only = FALSE)
on.exit(DBI::dbDisconnect(persist_con, shutdown = TRUE), add = TRUE)

metadata <- data.frame(
  key = c("build_date", "ecotox_release", "ecotox_release_date",
