...
test_that("cc_detail passes request metadata to helper", {
  ...
  result <- try(suppressWarnings(suppressMessages(ComptoxR::cc_detail(cas_rn = "80-05-7"))), silent = TRUE)
  expect_gt(length(calls), 0L)
  call <- calls[[1L]]
  expect_true(is.list(call))
  expect_equal(call$.helper, "generic_cc_request")
  expect_equal(call[["endpoint"]], "detail")
  expect_equal(call[["method"]], "GET")
  expect_equal(call[["cas_rn"]], "80-05-7")
})
