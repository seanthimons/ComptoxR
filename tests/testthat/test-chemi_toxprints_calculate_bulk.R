# Tests for chemi_toxprints_calculate_bulk
# Generated by dev/generate_tests.R; do not edit by hand.
# Generator: offline wrapper contract tests

test_that("chemi_toxprints_calculate_bulk passes request metadata to helper", {
  ...
  result <- try(suppressWarnings(suppressMessages(ComptoxR::chemi_toxprints_calculate_bulk(query = "DTXSID7020182"))), silent = TRUE)
  expect_gt(length(calls), 0L)
  call <- calls[[1L]]
  expect_true(is.list(call))
  expect_equal(call$.helper, "generic_chemi_request")
  expect_null(call[["query"]])
  expect_equal(call[["endpoint"]], "toxprints/calculate")
