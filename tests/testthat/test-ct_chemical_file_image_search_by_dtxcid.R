...
test_that("ct_chemical_file_image_search_by_dtxcid passes request metadata to helper", {
  ...
  result <- try(suppressWarnings(suppressMessages(ComptoxR::ct_chemical_file_image_search_by_dtxcid(dtxcid = "DTXCID30182"))), silent = TRUE)
  expect_gt(length(calls), 0L)
  call <- calls[[1L]]
  expect_true(is.list(call))
  expect_equal(call$.helper, "generic_request")
  expect_equal(call[["query"]], "DTXCID30182")
  expect_equal(call[["endpoint"]], "chemical/file/image/search/by-dtxcid/")
  expect_equal(call[["method"]], "GET")
  expect_equal(call[["batch_limit"]], 1)
  expect_equal(call[["content_type"]], "image/png")
})
