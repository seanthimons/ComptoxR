...
testthat::test_that("ct_exposure_httk_search_bulk completes the fixed call sequence", {
  contract <- readRDS(testthat::test_path("fixtures/specmill-full-contracts.rds"))[["ct_exposure_httk_search_bulk"]]
  ...
  result <- do.call(ComptoxR::ct_exposure_httk_search_bulk, contract$inputs)
