...
test_that("ct_bioactivity_aop_search_by_entrez_gene_id passes request metadata to helper", {
  ...
  result <- try(suppressWarnings(suppressMessages(ComptoxR::ct_bioactivity_aop_search_by_entrez_gene_id(entrezGeneId = 1L))), silent = TRUE)
  expect_gt(length(calls), 0L)
  call <- calls[[1L]]
  expect_true(is.list(call))
  expect_equal(call$.helper, "generic_request")
  expect_equal(call[["query"]], 1L)
  expect_equal(call[["endpoint"]], "bioactivity/aop/search/by-entrez-gene-id/")
