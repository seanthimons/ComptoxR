# Tests for chemi_amos_method_keyset_pagination_bulk_development
# Generated by dev/generate_tests.R; do not edit by hand.
# Generator: offline wrapper contract tests

test_that("chemi_amos_method_keyset_pagination_bulk_development passes request metadata to helper", {
  ...
  result <- try(suppressWarnings(suppressMessages(ComptoxR::chemi_amos_method_keyset_pagination_bulk_development(limit = 1L))), silent = TRUE)
  expect_gt(length(calls), 0L)
  call <- calls[[1L]]
  expect_true(is.list(call))
  expect_equal(call$.helper, "generic_request")
  expect_null(call[["query"]])
  expect_equal(call[["endpoint"]], "amos/method_keyset_pagination/")
