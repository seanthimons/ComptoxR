...
testthat::test_that("chemi_alerts_alerts completes the fixed call sequence", {
  contract <- readRDS(testthat::test_path("fixtures/specmill-pilot-contracts.rds"))[["chemi_alerts_alerts"]]
  captured <- list()
  ...
  result <- do.call(ComptoxR::chemi_alerts_alerts, contract$inputs)
