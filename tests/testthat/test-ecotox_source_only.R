...
test_that("shipped localhost Plumber results match direct source-only queries", {
  ...
  tables <- eco_tables(con = con)
  fields <- eco_fields("results", con = con)
  inventory <- eco_inventory(con = con)
  species <- eco_species("rainbow%", con = con)
  ...
  expect_setequal(eco_tables(), tables)
  expect_identical(eco_fields("results"), fields)
  expect_equal(eco_inventory(), inventory)
  expect_equal(eco_species("rainbow%"), species)
  expect_equal(eco_species("Oncorhynchus%", field = "latin_name"), species)
  expect_equal(eco_species("Fish", field = "eco_group"), species)
  ...
  server$kill()
  expect_error(eco_results(casrn = "50-29-3"), "ECOTOX Plumber request failed")
})

test_that("native ECOTOX inspection preserves source records and reports metadata", {
  path <- make_source_ecotox_db()
  withr::defer(unlink(path))
  con <- DBI::dbConnect(duckdb::duckdb(), dbdir = path)
  withr::defer(DBI::dbDisconnect(con, shutdown = TRUE))
  withr::local_envvar(c(eco_burl = path))
  expect_equal(eco_inventory(con = con)$cas_number, "50293")
  expect_equal(eco_inventory(con = con)$chemical_name, "DDT")
  expect_true(all(c("tests", "results", "species", "lifestage_codes") %in% eco_tables(con = con)))
  expect_identical(eco_fields("lifestage_codes", con = con), c("code", "description"))
  expect_equal(eco_species("rainbow%", con = con)$latin_name, "Oncorhynchus mykiss")
  expect_equal(eco_species("Oncorhynchus%", field = "latin_name", con = con)$common_name, "Rainbow Trout")
  expect_equal(eco_species("Fish", field = "eco_group", con = con)$species_number, 1L)
  expect_equal(nrow(eco_species("absent", con = con)), 0L)
  expect_equal(nrow(eco_species("%' OR 1=1 --", con = con)), 0L)
  expect_error(eco_fields("", con = con), "single non-empty character")
  expect_error(eco_fields(c("tests", "results"), con = con), "single non-empty character")
  expect_error(eco_species("x", field = "invalid", con = con), "should be one of")
  metadata <- DBI::dbReadTable(con, "_metadata")
  metadata <- rbind(metadata, data.frame(key = "ecotox_release_date", value = "2026-03-12"))
  DBI::dbWriteTable(con, "_metadata", metadata, overwrite = TRUE)
  expect_identical(eco_health(con = con)$version_date, "2026-03-12")
  DBI::dbRemoveTable(con, "_metadata")
  expect_identical(eco_health(con = con)$status, "ok")
  expect_true(is.na(eco_health(con = con)$version_date))
})

test_that("source-only result filters combine names, flags, endpoints, and extra columns", {
  path <- make_source_ecotox_db()
  withr::defer(unlink(path))
  con <- DBI::dbConnect(duckdb::duckdb(), dbdir = path)
  withr::defer(DBI::dbDisconnect(con, shutdown = TRUE))
  withr::local_envvar(c(eco_burl = path))
  tests <- DBI::dbReadTable(con, "tests")[rep(1L, 4L), ]
  tests$test_id <- tests$species_number <- seq_len(4L)
  tests$test_note <- paste0("test ", seq_len(4L))
  results <- DBI::dbReadTable(con, "results")[rep(1L, 4L), ]
  results$test_id <- results$result_id <- seq_len(4L)
  results$endpoint <- c("LC50", "EC50", "SPECIAL", "XNOEC")
  results$result_note <- paste0("result ", seq_len(4L))
  species <- DBI::dbReadTable(con, "species")[rep(1L, 4L), ]
  species$species_number <- seq_len(4L)
  species$common_name <- c("Rainbow Trout", "Water Flea", "Duckweed", "Minnow")
  species$latin_name <- c("Oncorhynchus mykiss", "Daphnia magna", "Lemna minor", "Pimephales promelas")
  species$eco_group <- c("Fish", "Invertebrates", "Plants", "Fish")
  species$standard_test_species <- c(TRUE, FALSE, FALSE, FALSE)
  species$invasive_species <- c(FALSE, TRUE, FALSE, FALSE)
  species$endangered_threatened_species <- c(FALSE, FALSE, TRUE, FALSE)
  DBI::dbWriteTable(con, "tests", tests, overwrite = TRUE)
  DBI::dbWriteTable(con, "results", results, overwrite = TRUE)
  DBI::dbWriteTable(con, "species", species, overwrite = TRUE)
  expect_setequal(eco_results(common_name = "Rainbow Trout", latin_name = "Daphnia magna", con = con)$result_id, 1:2)
  expect_identical(eco_results(latin_name = "Lemna minor", con = con)$result_id, 3L)
  expect_setequal(eco_results(eco_group = "Fish", con = con)$result_id, c(1L, 4L))
  expect_identical(eco_results(standard = TRUE, con = con)$result_id, 1L)
  expect_identical(eco_results(invasive = TRUE, con = con)$result_id, 2L)
  expect_identical(eco_results(threatened = TRUE, con = con)$result_id, 3L)
  expect_setequal(eco_results(endpoint = "all", con = con)$result_id, 1:4)
  expect_setequal(eco_results(endpoint = "default", con = con)$result_id, 1:2)
  expect_setequal(eco_results(endpoint = c("LC50", "SPECIAL"), con = con)$result_id, c(1L, 3L))
  selected <- eco_results(
    common_name = "Rainbow Trout",
    test_cols = "test_note",
    results_cols = "result_note",
    con = con
  )
  expect_identical(selected$test_note, "test 1")
  expect_identical(selected$result_note, "result 1")
  expect_identical(selected$organism_lifestage, "L001")
  expect_identical(selected$org_lifestage, "Adult")
  expect_equal(nrow(eco_results(common_name = "Rainbow Trout", endpoint = "SPECIAL", con = con)), 0L)
})
