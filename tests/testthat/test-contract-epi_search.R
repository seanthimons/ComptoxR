...
testthat::test_that("epi_search completes the fixed call sequence", {
  contract <- readRDS(testthat::test_path("fixtures/specmill-pilot-contracts.rds"))[["epi_search"]]
  captured <- list()
  ...
  result <- do.call(ComptoxR::epi_search, contract$inputs)
