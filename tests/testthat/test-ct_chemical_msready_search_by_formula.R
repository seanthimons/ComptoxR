...
test_that("ct_chemical_msready_search_by_formula passes request metadata to helper", {
  ...
  result <- try(suppressWarnings(suppressMessages(ComptoxR::ct_chemical_msready_search_by_formula(formula = "C6H6"))), silent = TRUE)
  expect_gt(length(calls), 0L)
  call <- calls[[1L]]
  expect_true(is.list(call))
  expect_equal(call$.helper, "generic_request")
  expect_equal(call[["query"]], "C6H6")
  expect_equal(call[["endpoint"]], "chemical/msready/search/by-formula/")
