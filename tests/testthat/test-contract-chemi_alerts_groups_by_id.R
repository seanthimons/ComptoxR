...
testthat::test_that("chemi_alerts_groups_by_id completes the fixed call sequence", {
  contract <- readRDS(testthat::test_path("fixtures/specmill-pilot-contracts.rds"))[["chemi_alerts_groups_by_id"]]
  captured <- list()
  ...
  result <- do.call(ComptoxR::chemi_alerts_groups_by_id, contract$inputs)
