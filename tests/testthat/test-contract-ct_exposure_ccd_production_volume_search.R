...
testthat::test_that("ct_exposure_ccd_production_volume_search completes the fixed call sequence", {
  contract <- readRDS(testthat::test_path("fixtures/specmill-full-contracts.rds"))[[
    "ct_exposure_ccd_production_volume_search"
  ]]
  ...
  result <- do.call(ComptoxR::ct_exposure_ccd_production_volume_search, contract$inputs)
