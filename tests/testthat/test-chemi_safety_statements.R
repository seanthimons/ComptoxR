...
test_that("chemi_safety_statements passes request metadata to helper", {
  ...
  result <- try(suppressWarnings(suppressMessages(ComptoxR::chemi_safety_statements())), silent = TRUE)
  expect_gt(length(calls), 0L)
  call <- calls[[1L]]
  expect_true(is.list(call))
  expect_equal(call$.helper, "generic_request")
  expect_equal(call[["endpoint"]], "safety/statements")
