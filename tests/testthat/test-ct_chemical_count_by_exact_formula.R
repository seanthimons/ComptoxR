...
test_that("ct_chemical_count_by_exact_formula passes request metadata to helper", {
  ...
  result <- try(suppressWarnings(suppressMessages(ComptoxR::ct_chemical_count_by_exact_formula(formula = "C6H6"))), silent = TRUE)
  expect_gt(length(calls), 0L)
  call <- calls[[1L]]
  expect_true(is.list(call))
  expect_equal(call$.helper, "generic_request")
  expect_equal(call[["query"]], "C6H6")
  expect_equal(call[["endpoint"]], "chemical/count/by-exact-formula/")
