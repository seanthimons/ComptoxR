...
test_that("chemi_alerts_operations passes request metadata to helper", {
  ...
  result <- try(suppressWarnings(suppressMessages(ComptoxR::chemi_alerts_operations())), silent = TRUE)
  expect_gt(length(calls), 0L)
  call <- calls[[1L]]
  expect_true(is.list(call))
  expect_equal(call$.helper, "generic_request")
  expect_equal(call[["endpoint"]], "alerts/operations")
