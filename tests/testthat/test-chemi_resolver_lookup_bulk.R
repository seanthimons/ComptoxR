# Tests for chemi_resolver_lookup_bulk
# Generated by dev/generate_tests.R; do not edit by hand.
# Generator: offline wrapper contract tests

test_that("chemi_resolver_lookup_bulk passes request metadata to helper", {
  ...
  result <- try(suppressWarnings(suppressMessages(ComptoxR::chemi_resolver_lookup_bulk(ids = "DTXSID7020182"))), silent = TRUE)
  ...
  expect_equal(call[["endpoint"]], "resolver/lookup")
  expect_true("options" %in% names(call))
  expect_true(is.null(call[["options"]]) || length(call[["options"]]) >= 0)
  expect_equal(call[["sid_label"]], "ids")
  expect_equal(call[["array_payload"]], TRUE)
})
