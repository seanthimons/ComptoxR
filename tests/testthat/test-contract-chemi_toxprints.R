...
testthat::test_that("chemi_toxprints completes the fixed call sequence", {
  contract <- readRDS(testthat::test_path("fixtures/specmill-rebuild-contracts.rds"))[["chemi_toxprints"]]
  ...
  result <- do.call(ComptoxR::chemi_toxprints, contract$inputs)
