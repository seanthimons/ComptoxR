# Tests for cts_metabolizer_inputs
# Generated by dev/generate_tests.R; do not edit by hand.
# Generator: offline wrapper contract tests

test_that("cts_metabolizer_inputs passes request metadata to helper", {
  ...
  mock_helper <- function(...) {
    captured <- list(...)
    captured$.helper <- "generic_cts_request"
    calls[[length(calls) + 1L]] <<- captured
    generated_contract_response(...)
  }

  testthat::local_mocked_bindings(
    generic_cts_request = mock_helper,
    .package = "ComptoxR"
  )

  result <- try(suppressWarnings(suppressMessages(ComptoxR::cts_metabolizer_inputs())), silent = TRUE)
  expect_gt(length(calls), 0L)
  call <- calls[[1L]]
  expect_true(is.list(call))
  expect_equal(call$.helper, "generic_cts_request")
  expect_equal(call[["endpoint"]], "metabolizer/inputs")
