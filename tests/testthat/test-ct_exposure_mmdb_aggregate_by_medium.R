...
test_that("ct_exposure_mmdb_aggregate_by_medium passes request metadata to helper", {
  ...
  result <- try(suppressWarnings(suppressMessages(ComptoxR::ct_exposure_mmdb_aggregate_by_medium(medium = "water"))), silent = TRUE)
  expect_gt(length(calls), 0L)
  call <- calls[[1L]]
  expect_true(is.list(call))
  expect_equal(call$.helper, "generic_request")
  expect_equal(call[["endpoint"]], "exposure/mmdb/aggregate/by-medium")
