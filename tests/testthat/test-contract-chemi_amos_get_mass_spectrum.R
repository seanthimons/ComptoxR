...
testthat::test_that("chemi_amos_get_mass_spectrum completes the fixed call sequence", {
  contract <- readRDS(testthat::test_path("fixtures/specmill-pilot-contracts.rds"))[["chemi_amos_get_mass_spectrum"]]
  captured <- list()
  ...
  result <- do.call(ComptoxR::chemi_amos_get_mass_spectrum, contract$inputs)
