...
test_that("ct_bioactivity_analyticalqc works with valid input", 
    {
        vcr::use_cassette("ct_bioactivity_analyticalqc_query", 
            {
                result <- ct_bioactivity_analyticalqc(query = "DTXSID7020182")
                {
...
test_that("ct_bioactivity_analyticalqc handles batch requests", 
    {
        vcr::use_cassette("ct_bioactivity_analyticalqc_batch", 
            {
                result <- ct_bioactivity_analyticalqc(query = c("DTXSID7020182", 
                "DTXSID5032381"))
                expect_s3_class(result, "tbl_df")
...
test_that("ct_bioactivity_analyticalqc handles invalid input gracefully", 
    {
        vcr::use_cassette("ct_bioactivity_analyticalqc_error", 
            {
                expect_warning(result <- ct_bioactivity_analyticalqc(query = "INVALID_ID"))
