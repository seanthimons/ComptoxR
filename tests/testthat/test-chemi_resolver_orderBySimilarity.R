...
test_that("chemi_resolver_orderBySimilarity passes request metadata to helper", {
  ...
  result <- try(suppressWarnings(suppressMessages(ComptoxR::chemi_resolver_orderBySimilarity(query = "DTXSID7020182"))), silent = TRUE)
  expect_gt(length(calls), 0L)
  call <- calls[[1L]]
  expect_true(is.list(call))
  expect_equal(call$.helper, "generic_chemi_request")
  expect_null(call[["query"]])
  expect_equal(call[["endpoint"]], "resolver/orderBySimilarity")
