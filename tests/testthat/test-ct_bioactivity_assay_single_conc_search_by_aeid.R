...
test_that("ct_bioactivity_assay_single_conc_search_by_aeid passes request metadata to helper", {
  ...
  result <- try(suppressWarnings(suppressMessages(ComptoxR::ct_bioactivity_assay_single_conc_search_by_aeid(aeid = 1L))), silent = TRUE)
  expect_gt(length(calls), 0L)
  call <- calls[[1L]]
  expect_true(is.list(call))
  expect_equal(call$.helper, "generic_request")
  expect_equal(call[["query"]], 1L)
  expect_equal(call[["endpoint"]], "bioactivity/assay/single-conc/search/by-aeid/")
