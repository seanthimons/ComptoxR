...
test_that("ct_hazard_toxref_search_by_study_id passes request metadata to helper", {
  ...
  result <- try(suppressWarnings(suppressMessages(ComptoxR::ct_hazard_toxref_search_by_study_id(studyId = 1L))), silent = TRUE)
  expect_gt(length(calls), 0L)
  call <- calls[[1L]]
  expect_true(is.list(call))
  expect_equal(call$.helper, "generic_request")
  expect_equal(call[["query"]], 1L)
  expect_equal(call[["endpoint"]], "hazard/toxref/summary/search/by-study-id/")
