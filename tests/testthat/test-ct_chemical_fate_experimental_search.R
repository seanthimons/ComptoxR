...
test_that("ct_chemical_fate_experimental_search passes request metadata to helper", {
  ...
  result <- try(suppressWarnings(suppressMessages(ComptoxR::ct_chemical_fate_experimental_search(dtxsid = "DTXSID7020182", propName = "DTXSID7020182"))), silent = TRUE)
  expect_gt(length(calls), 0L)
  call <- calls[[1L]]
  expect_true(is.list(call))
  expect_equal(call$.helper, "generic_request")
  expect_equal(call[["endpoint"]], "chemical/fate/summary/experimental/search/")
