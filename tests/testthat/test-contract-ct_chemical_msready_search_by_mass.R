...
testthat::test_that("ct_chemical_msready_search_by_mass completes the fixed call sequence", {
  contract <- readRDS(testthat::test_path("fixtures/specmill/ctx.rds"))[["ct_chemical_msready_search_by_mass"]]
  ...
  result <- do.call(ComptoxR::ct_chemical_msready_search_by_mass, contract$inputs)
