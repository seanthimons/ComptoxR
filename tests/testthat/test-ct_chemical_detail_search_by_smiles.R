...
test_that("ct_chemical_detail_search_by_smiles passes request metadata to helper", {
  ...
  result <- try(suppressWarnings(suppressMessages(ComptoxR::ct_chemical_detail_search_by_smiles(smiles = "CCCC"))), silent = TRUE)
  expect_gt(length(calls), 0L)
  call <- calls[[1L]]
  expect_true(is.list(call))
  expect_equal(call$.helper, "generic_request")
  expect_equal(call[["endpoint"]], "chemical/detail/search/by-smiles/")
  expect_equal(call[["method"]], "GET")
  expect_equal(call[["batch_limit"]], 0)
  expect_equal(call[["smiles"]], "CCCC")
})
