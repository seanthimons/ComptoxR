...
testthat::test_that("ct_chemical_search_by_msready_formula completes the fixed call sequence", {
  contract <- readRDS(testthat::test_path("fixtures/specmill-full-contracts.rds"))[[
    "ct_chemical_search_by_msready_formula"
  ]]
  ...
  result <- do.call(ComptoxR::ct_chemical_search_by_msready_formula, contract$inputs)
